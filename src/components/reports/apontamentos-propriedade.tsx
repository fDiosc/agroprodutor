...
interface ApontamentosPropriedadeProps {
  esgData: Record<string, unknown> | null
  className?: string
}

export function ApontamentosPropriedade({
  esgData,
  className,
}: ApontamentosPropriedadeProps) {
  const totalApontamentos = (esgData?.total_apontamentos as number) ?? 0
  const hasApontamentos = totalApontamentos > 0

  return (
    <CollapsibleSection
      title="Apontamentos Propriedade"
      badge={
        <span
          className={cn(
